...
yq -i '.gateway.env.admin_gui_url = env(KONG_ADMIN_GUI_URL)' values.yaml
yq -i '.gateway.env.admin_gui_api_url = env(KONG_ADMIN_GUI_API_URL)' values.yaml
yq -i '.gateway.env.proxy_url = env(KONG_PROXY_URL)' values.yaml
