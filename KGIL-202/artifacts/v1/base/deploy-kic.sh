...
# Assumes KIND Cluster is running. KGLL-108
...
# helm repo update
# helm install -f /home/ubuntu/$KONG_COURSE_ID/exercises/monitoring/prometheus-values.yaml prometheus prometheus-community/kube-prometheus-stack -n monitoring --wait
# helm install -f /home/ubuntu/$KONG_COURSE_ID/exercises/monitoring/statsd-values.yaml statsd prometheus-community/prometheus-statsd-exporter -n monitoring --wait
...
kubectl -n kong create secret tls kong-portal-tls --key="/etc/kong/ssl/server.key" --cert="/etc/kong/ssl/server.crt"
...
yq -i '.env.admin_gui_url = env(KONG_ADMIN_GUI_URL)' ./base/cp-values.yaml
...
# Nginx Fix
kubectl -n kong create configmap nginx-kong-lua --from-file=base/nginx/nginx_kong.lua
kubectl -n kong create configmap nginx-kong-template --from-file=base/nginx/custom_nginx.template
